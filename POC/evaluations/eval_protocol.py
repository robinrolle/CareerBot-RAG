...
def evaluate_relevance(cv_text: str, items: List[str], item_type: str) -> Tuple[RelevanceEvaluation, int]:
    ...
    Evaluate the relevance and provide the count of relevant {item_type}s along with a brief explanation."""
...
def create_output_csv():
    headers = ['embedding_model', 'filename', 'max_retrieved', 'relevant_skills_count', 'skills_explanation',
               'relevant_occupations_count', 'occupations_explanation', 'total_tokens_used']
    ...
    with open(output_csv, 'a', newline='', encoding='utf-8') as file:
        ...
        for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Evaluating relevance"):
            ...
            writer.writerow([
                ...
                skills_evaluation.explanation,
                occupations_evaluation.relevant_count,
                occupations_evaluation.explanation,
                tokens_used
            ])
